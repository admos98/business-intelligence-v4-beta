    ...
    reader.onload = async (e) => {
      try {
        ...
        if (typeof result === 'string') {
          ...
          // Helper function to generate new ID with counter to ensure uniqueness
          let idCounter = 0;
          const generateId = () => `import-${Date.now()}-${++idCounter}-${Math.random().toString(36).substr(2, 9)}`;

          // Helper function to validate and clean option
          const cleanOption = (option: any): any => {
            if (!option || typeof option !== 'object') return null;
            if (!option.name || typeof option.name !== 'string') return null;
            ...
            if (option.label && typeof option.label === 'string') cleaned.label = option.label;
            if (typeof option.price === 'number') cleaned.price = option.price;
            if (typeof option.priceModifier === 'number') cleaned.priceModifier = option.priceModifier;
            if (typeof option.isCustomAmount === 'boolean') cleaned.isCustomAmount = option.isCustomAmount;
            if (option.unit && typeof option.unit === 'string') cleaned.unit = option.unit;
            if (typeof option.pricePerUnit === 'number') cleaned.pricePerUnit = option.pricePerUnit;

            return cleaned;
          };

          // Helper function to validate and clean variant/customization
          const cleanVariantOrCustomization = (vc: any): any => {
            if (!vc || typeof vc !== 'object') return null;
            if (!vc.name || typeof vc.name !== 'string') return null;
            if (!vc.type || !['text', 'number', 'choice'].includes(vc.type)) return null;
            ...
            if (typeof vc.required === 'boolean') cleaned.required = vc.required;

            // Clean options if present
            if (vc.options && Array.isArray(vc.options)) {
              const cleanedOptions = vc.options
                .map(cleanOption)
                .filter((opt: any) => opt !== null);
              if (cleanedOptions.length > 0) cleaned.options = cleanedOptions;
            }

            if (vc.unit && typeof vc.unit === 'string') cleaned.unit = vc.unit;
            if (typeof vc.pricePerUnit === 'number') cleaned.pricePerUnit = vc.pricePerUnit;
            if (typeof vc.priceModifier === 'number') cleaned.priceModifier = vc.priceModifier;

            return cleaned;
          };

          // Validate and clean items
          const validItems: Omit<POSItem, 'id'>[] = [];
          const errors: string[] = [];
          ...
          for (let i = 0; i < itemsToImport.length; i++) {
            ...
            if (typeof item.sellPrice !== 'number' || item.sellPrice < 0) {
              errors.push(`Item ${i + 1} (${item.name}): Missing or invalid sellPrice`);
              continue;
            }

            if (!item.category || typeof item.category !== 'string' || item.category.trim() === '') {
              errors.push(`Item ${i + 1} (${item.name}): Missing or invalid category`);
              continue;
            }

            // Build cleaned item (without id - will be generated by addPOSItem)
            ...
            // Optional fields
            if (item.recipeId && typeof item.recipeId === 'string') {
              cleanedItem.recipeId = item.recipeId;
            }
            if (typeof item.isTaxable === 'boolean') {
              cleanedItem.isTaxable = item.isTaxable;
            }
            if (item.taxRateId && typeof item.taxRateId === 'string') {
              cleanedItem.taxRateId = item.taxRateId;
            }
            if (typeof item.order === 'number') {
              cleanedItem.order = item.order;
            }

            // Clean variants
            if (item.variants && Array.isArray(item.variants)) {
              const cleanedVariants = item.variants
                .map(cleanVariantOrCustomization)
                .filter((v: any) => v !== null);
              if (cleanedVariants.length > 0) {
                cleanedItem.variants = cleanedVariants;
              }
            }

            // Clean customizations
            if (item.customizations && Array.isArray(item.customizations)) {
              const cleanedCustomizations = item.customizations
                .map(cleanVariantOrCustomization)
                .filter((c: any) => c !== null);
              if (cleanedCustomizations.length > 0) {
                cleanedItem.customizations = cleanedCustomizations;
              }
            }
          ...
          if (validItems.length === 0) {
            const errorMsg = errors.length > 0
              ? `هیچ کالای معتبری یافت نشد. خطاها: ${errors.slice(0, 3).join('; ')}${errors.length > 3 ? '...' : ''}`
              : 'هیچ کالای معتبری در فایل یافت نشد';
            addToast(errorMsg, 'error');
            return;
          }

          if (errors.length > 0) {
            addToast(`${validItems.length} کالا وارد شد، ${errors.length} مورد رد شد`, 'warning');
          }
          ...
          validItems.forEach((item, index) => {
            ...
            // Update order if specified, otherwise use calculated order
            const finalOrder = order !== undefined ? order : maxOrder + index + 1;
            updatePOSItem(newId, { order: finalOrder });
          });
        ...
        addToast(`خطا در وارد کردن فایل: ${errorMessage}`, 'error');
