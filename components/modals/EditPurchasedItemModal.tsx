...
const EditPurchasedItemModal: React.FC<EditPurchasedItemModalProps> = ({ item, onClose, onSave }) => {
  ...
  const [vendor, setVendor] = useState(vendors.find(v => v.id === item.vendorId)?.name || '');
  const [paymentMethod, setPaymentMethod] = useState<PaymentMethod>(item.paymentMethod || PaymentMethod.Card);
  const [paymentStatus, setPaymentStatus] = useState<PaymentStatus>(item.paymentStatus || PaymentStatus.Paid);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (purchasedAmount !== '' && Number(purchasedAmount) > 0 && pricePerUnit !== '' && Number(pricePerUnit) >= 0) {
      const vendorId = findOrCreateVendor(vendor);
      onSave(item.id, {
  ...
  return (
    <div className={`fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4 transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'opacity-0'}`}>
      <div className={`bg-surface p-6 rounded-xl border border-border w-full max-w-md transition-all duration-300 ${isOpen ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-10'}`}>
        <h2 className="text-xl font-bold text-primary mb-2">{t.editPurchasedItemTitle}</h2>
